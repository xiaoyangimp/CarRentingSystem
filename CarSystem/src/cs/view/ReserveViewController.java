...
//		returncal.set(Calendar.MILLISECOND, 0);
		...
		if(pickcal.before(Calendar.getInstance()) || (!pickcal.before(returncal)) ) {
		...
		try{
			/* query that obtain a vehicle that is available during the customer's request time*/
	...
	protected void confirm(){
		...
		try {
			...
			con.setReservation(this.res, type.getSelectionModel().getSelectedItem().toString());
